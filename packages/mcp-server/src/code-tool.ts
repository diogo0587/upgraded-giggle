...
import { ClientOptions } from 'testedsj';

const prompt = `Runs JavaScript code to interact with the Testedsj API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    TESTEDSJ_API_KEY: requireValue(
      readEnv('TESTEDSJ_API_KEY') ?? client.apiKey,
      'set TESTEDSJ_API_KEY environment variable or provide apiKey client option',
    ),
    TESTEDSJ_BASE_URL: readEnv('TESTEDSJ_BASE_URL') ?? client.baseURL ?? undefined,
  };
